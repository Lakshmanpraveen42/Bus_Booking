    ...
    <AdminLayout title="Operations Overview">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  ...
                  <tr key={booking.id} className="group hover:bg-slate-50/50 transition-colors">
                    <td className="py-5 font-bold text-slate-900 text-sm">BGO-{booking.id * 123}</td>
                    <td className="py-5">
                    ...
                    <td className="py-5 text-sm text-slate-600 font-medium">Trip: {booking.trip_id}</td>
                    <td className="py-5">
                      <Badge variant={booking.status === 'booked' ? 'success' : 'danger'}>
                        {booking.status}
                      </Badge>
                    </td>
                    <td className="py-5 text-right font-black text-slate-900">{formatPrice(booking.total_amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="lg:col-span-1 space-y-8">
          <div className="bg-slate-950 rounded-[40px] p-8 text-white relative overflow-hidden shadow-2xl shadow-slate-950/20">
